...
- **`golden-harness`** (`tests/golden_harness`) — 24-task acceptance eval. Run with `cargo run -p golden-harness`.
...
- **Golden harness on default Linux scores 16/24 — this is correct, not a regression.** `FS-02`/`SB-01` require Darwin; `ROUT-01`, `MEM-01`, `GRAPH-01`, `LOOP-02`, `PLAN-01`, and `LOOP-04` require Darwin/Ollama and print `FAIL-CLOSED`. With local Ollama + MCP the verified score is 21/24 (FS-02, SB-01, and OS-gated LOOP-02 fail closed). See `docs/LINUX_CI.md`.
