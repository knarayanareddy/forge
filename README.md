...
the harness has since grown to 30 tasks with **UNDO-01** (Phase 9 slices 9.7-9.8), **LOOP-04**
...
apply), **PERM-02** (Phase 10 slices 10.8-10.9, batched approval gate), **SUB-01** (Phase 10
slices 10.3-10.4, subagent delegation), and **SEC-01** (Phase 11 slice 11.6, brokered secrets),
all Linux-verified but not yet re-run through canonical Darwin CI.

## Harness score (Darwin, canonical)

```text
cargo run -p golden-harness --bin golden-harness
→ 30/30 harness target (Phase 8.0 closure verified 22/22 at 432ace9; UNDO-01, LOOP-04, HOOK-01,
  CKPT-01, CONS-01, PERM-02, SUB-01, and SEC-01 added after and are Linux-verified pending their
  first Darwin canonical run) when ROUT-01 median warm TTFT ≤ 200ms,
...
|----------|----------------|-------------|
| **Darwin** (Ollama + `sandbox-exec`) | **30/30 target** | 22/22 verified through `432ace9` (run `30565128737`); later slices Linux-verified pending Darwin re-run |
