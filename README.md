...
the harness has since grown to 27 tasks with **UNDO-01** (Phase 9 slices 9.7-9.8), **LOOP-04**
(Phase 9 slices 9.9-9.10), **HOOK-01** (Phase 10 slice 10.7, `PreToolUse` hooks), **CKPT-01**
(Phase 10 slice 10.1, checkpoint + rewind), and **CONS-01** (Phase 11, human-in-loop consolidation
apply), all Linux-verified but not yet re-run through canonical Darwin CI.

## Harness score (Darwin, canonical)

```text
cargo run -p golden-harness --bin golden-harness
→ 27/27 harness target (Phase 8.0 closure verified 22/22 at 432ace9; UNDO-01, LOOP-04, HOOK-01,
  CKPT-01, and CONS-01 added after and are Linux-verified pending their first Darwin canonical run)
  when ROUT-01 median warm TTFT ≤ 200ms,
...
|----------|----------------|-------------|
| **Darwin** (Ollama + `sandbox-exec`) | **27/27 target** | 22/22 verified through `432ace9` (run `30565128737`); UNDO-01, LOOP-04, HOOK-01, CKPT-01, CONS-01 added since, Linux-verified |
| **Linux CI** (full matrix) | **19/27** | 19 hard · FS-02, SB-01, MEM-01, ROUT-01, GRAPH-01, LOOP-02, PLAN-01, LOOP-04 **FAIL-CLOSED** |
| **Linux Ollama-independent** | **19/19** | FS-01, SAFE-01, RES-01, GIT-01, CODE-01, MCP-01, MEM-02, SKILL-01, SKILL-02, RED-01, LOOP-01, SESS-01, **UNDO-01**, AUTO-01, CHECK-01, GATE-01, **HOOK-01**, **CKPT-01**, **CONS-01** |

Tasks (27): ROUT-01, FS-01, FS-02, **SB-01**, GIT-01, CODE-01, MCP-01, MEM-01, **MEM-02**, GRAPH-01, SKILL-01, SKILL-02, SAFE-01, RED-01, RES-01, LOOP-01, LOOP-02, **PLAN-01**, **LOOP-04**, **SESS-01**, **UNDO-01**, **AUTO-01**, **CHECK-01**, **GATE-01**, **HOOK-01**, **CKPT-01**, **CONS-01**
...
| **10 — Product Surface** | Checkpoint + rewind (CKPT-01), `PreToolUse` hook that blocks (HOOK-01) | **Slices 10.1 and 10.7 implemented ahead of the rest of Phase 10 (FORK-01, SUB-01, PERM-02 not yet started); Linux-verified** |
...
- **CI:** `.github/workflows/ci.yml` — Linux full matrix gate **≥19/27** · Darwin PR **build + unit tests + Swift only** · push/nightly Darwin **27/27 target** ([docs/LINUX_CI.md](docs/LINUX_CI.md))
