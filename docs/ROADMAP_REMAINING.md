...
**Harness registry:** `tests/golden_harness/src/main.rs` — **61 tasks** (50 hard / 11 soft)
...
All planned 1.0 engineering slices are on `main`. The golden harness registry was frozen at **51 tasks** at closure; wave 1 of [REVIEW_FABLE51_HARNESS.md](./REVIEW_FABLE51_HARNESS.md) then added **CHECK-02**, **GATE-03**, and **RED-02**, wave 2 added **LOOP-05**, **READ-01**, **REPLY-01**, and **PLAN-02**, and wave 3 has added **MEM-04**, **COMPACT-02** and **GATE-04**, so it stands at **61 tasks** and Darwin push/nightly CI enforces **61/61** (50 hard / 11 soft). Distribution scripts, Sparkle wiring, and DIST-01 gates are implemented — only **maintainer credentials** and **manual QA** block public install claims.
...
| Metric | Value |
|--------|-------|
| **Harness registry** | **61 tasks** (50 hard / 11 soft) |
| **main HEAD** | `7486762` — Merge PR #49 (final closure docs + harness flake hardening) |
| **Darwin CI gate** | Push/nightly **61/61** (`.github/workflows/ci.yml`) |
| **Last cited full Darwin run** | **38/38 @ `6caa521`** (PR #39); tasks 39–61 merged afterward — fresh **61/61** local cite recommended (CHECK-02 / GATE-03 / RED-02 / LOOP-05 / READ-01 / REPLY-01 / PLAN-02 / MEM-04 / COMPACT-02 / GATE-04 have never been included in a cited run) |
| **Scoreboard script** | `./scripts/check-doc-scoreboard.sh` — **PASS** |
| **Open PRs** | **0** |

### Harness tasks on `main` (61)
...
**Soft on Darwin (11):** REG-01, SLEEP-01, RELY-01, FORENSIC-01, MCP-02, COMPACT-01, HOOK-02, MEM-03, MCPS-01, OFFLINE-01, GATE-04.
...
|-----------|-------|-------|
| Harness / eval culture | **9.5 / 10** | 61 tasks; 50 hard + 11 soft; anti-theater CI; scoreboard script |
...
./scripts/check-doc-scoreboard.sh   # expect PASS (61 tasks)
