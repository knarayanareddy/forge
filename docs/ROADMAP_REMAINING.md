...
**Harness registry:** `tests/golden_harness/src/main.rs` — **58 tasks** (48 hard / 10 soft)
...
All planned 1.0 engineering slices are on `main`. The golden harness registry was frozen at **51 tasks** at closure; wave 1 of [REVIEW_FABLE51_HARNESS.md](./REVIEW_FABLE51_HARNESS.md) then added **CHECK-02**, **GATE-03**, and **RED-02**, and wave 2 added **LOOP-05**, **READ-01**, **REPLY-01**, and **PLAN-02**, so it stands at **58 tasks** and Darwin push/nightly CI enforces **58/58** (48 hard / 10 soft). Distribution scripts, Sparkle wiring, and DIST-01 gates are implemented — only **maintainer credentials** and **manual QA** block public install claims.
...
| Metric | Value |
|--------|-------|
| **Harness registry** | **58 tasks** (48 hard / 10 soft) |
| **main HEAD** | `7486762` — Merge PR #49 (final closure docs + harness flake hardening) |
| **Darwin CI gate** | Push/nightly **58/58** (`.github/workflows/ci.yml`) |
| **Last cited full Darwin run** | **38/38 @ `6caa521`** (PR #39); tasks 39–58 merged afterward — fresh **58/58** local cite recommended (CHECK-02 / GATE-03 / RED-02 / LOOP-05 / READ-01 / REPLY-01 / PLAN-02 have never been included in a cited run) |
| **Scoreboard script** | `./scripts/check-doc-scoreboard.sh` — **PASS** |
| **Open PRs** | **0** |

### Harness tasks on `main` (58)
...
|-----------|-------|-------|
| Harness / eval culture | **9.5 / 10** | 58 tasks; 48 hard + 10 soft; anti-theater CI; scoreboard script |
...
./scripts/check-doc-scoreboard.sh   # expect PASS (58 tasks)
