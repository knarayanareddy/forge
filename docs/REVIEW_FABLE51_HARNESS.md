...
| **P2-14** | `aether_core::gate_mode`: `GateMode::{Enforce, Log}`, a `GateSpec` parsed from `AETHER_GATE_MODE=<gate>:log\|enforce`, `GateHit { gate, would_deny, detail }`, a process-wide sink (`record_gate_hit` / `gate_hits` / `drain_gate_hits`), and the single seam `moderate_denial(gate, detail)` called at a gate's deny site — `None` means enforce and records nothing, `Some(hit)` means the would-be denial was recorded and the caller allows. Three properties make it safe rather than merely convenient: **fail-safe configuration** (absent, empty or unparsable spec ⇒ everything enforces, and the rejection is printed and surfaced via `GateSpec::rejected`), **a closed gate list** (`DARK_LAUNCHABLE_GATES` — a typo is refused with the real names, and one bad entry invalidates the whole spec, because a half-applied safety config is worse than either extreme), and **`NEVER_DARK_LAUNCHABLE`** naming `hook.output_redaction` with the reason: logging instead of redacting *is* the leak. Measurement is the point, so `GateLedger` pairs every decision with a `CorpusLabel`, and `GateSummary` reports `false_positives` (would-deny on benign) and `misses` (allow on adversarial) per gate; `ready_to_enforce()` is the promotion criterion and treats `observed == 0` as *no evidence*, not clean. Wired at both hook deny sites and at `admit_plan_against_observations`. | `GATE-04` (soft) — spec parsing including every refusal path; enforce-by-default measured over a frozen **labelled** corpus (4 hook cases × benign/adversarial, plus all fifteen INJECT-01 plan cases read from `inject01_corpus.json` rather than duplicated, so the criterion is measured on the corpus the gate is already frozen against); log mode records exactly one would-deny per adversarial input with triageable detail and nothing at all for benign ones; enforce mode writes no telemetry; the promotion verdict flips to **not ready** on one injected false positive *and* on one injected miss; and the environment is restored by an RAII guard, asserted by re-running an adversarial input after the guard drops. No model, no network. |
| **P1-5** | `FindingLeg { ContentCorrelation, PhraseInduction, AdversarialEscalation }` on every `CorrelationFinding`, plus the `step_index` it is about, so a finding states which leg produced it. `admit_plan_with_confirmation` layers a third outcome over the *unchanged* two-valued gate: when every finding is content correlation, the refusal becomes `AdmitOutcome::RequireApproval` carrying one `ApprovalRequest` per finding — induced step, inducing observation via `ApprovalRequest::render()`, still wrapped `trust="untrusted"` because a confirmation screen is untrusted-content-adjacent, and cut at `APPROVAL_EVIDENCE_MAX_CHARS` **with the cut announced** (P1-6 inside P1-5). Any phrase or adversarial-escalation finding keeps the hard deny. `truncate` now cuts by characters rather than bytes: `&s[..max]` could panic mid-character on a Unicode observation, which is a denial-of-service chosen by untrusted text. The finding's third ask — record every hit for false-positive measurement — had already landed with P2-14, since both lists are registered gates and their false-positive rate is a `GateSummary` rather than an opinion. **Deferred:** moving the lists into `profiles/`. They are `&'static` by construction (`HookRule` stores `&'static [&'static str]`), so file-backing them is a change to hook construction that cannot be compile-tested from this environment, and a runtime-loaded denylist that fails to load must fail *closed* — a decision worth making with a compiler in the room. | `INJECT-01` extended, not replaced: five `cohort: "paraphrase"` cases express the same induced-tool intent with **zero** of the 16 frozen phrases — alternate wording, split tokens (`ig nore prev ious`), Dutch, and an encoded payload laundered into a write — plus a benign control whose long token was in the goal all along. The task asserts that no frozen phrase matches those observations (walking the live `TOOL_RESULT_INJECTION_PATTERNS`, not a snapshot of it), that every finding carries `FindingLeg::ContentCorrelation`, that the outcome is `RequireApproval` with a renderable request whose evidence is still marked untrusted and whose bound is stated, and that `induce-mcp-via-phrase` still hard-denies. Widening the denylist therefore *fails* the task. Four unit tests pin the same properties in `aether-core`, including the Unicode-safe `truncate`. |

**Seven notes worth carrying forward.**
...
5. *Make the test unwidenable.* A gate test that goes green when the denylist grows is measuring the
   test author's diligence, not the defense. `INJECT-01`'s paraphrase cohort asserts its own silence
   against the live pattern list, so the only way to keep it green is for correlation to do the work —
   the anti-theater bar applied to the test itself. The same reasoning moved `GATE-04`'s expected counts
   from a literal `8` to a corpus-derived count: a hardcoded expectation makes every corpus addition
   look like a regression in the instrument.
6. *Denial and confirmation answer different questions.* "Is this attack-shaped?" is answerable by a
   rule; "is this what you asked for?" is answerable only by the person. Where the evidence is a
   specific step consuming specific untrusted text, the gate should hand the question over — and keep
   the hard deny for the legs whose evidence is a substring match, because showing a person a denylist
   hit is not showing them evidence.
7. *`log_line()` omits `attempts`.* Fix 2 asked for `Compacted { chars_before, chars_after, attempts,
