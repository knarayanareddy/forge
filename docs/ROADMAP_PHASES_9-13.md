...
**CKPT-01** (checkpoint + rewind), **PERM-02** (batched approval gate), and **SUB-01** (subagent
delegation) and Phase 11's **CONS-01** (consolidation apply/reject) have all been implemented ahead
of the rest of their respective phases — only `FORK-01` (a probe) remains in Phase 10 now —
bringing the harness to 29 tasks. **Darwin canonical re-verified 29/29 (29 hard / 0 soft)** on
`main` @ `51658d0`, run
[`30840008383`](https://github.com/knarayanareddy/forge/actions/runs/30840008383) — every slice
above passes for real on `macos-15` with live Ollama and Seatbelt, not just Linux fail-closed. A
small non-numbered gap flagged during the post-8.0 audit — `retrieve_session_memory` was wired into
`run_stream_task` (8.0b) but not the structured `nl:`-prefixed loop path — is also closed:
`run_nl_loop_task_with_replan` now recalls session memory before planning, fail-open on retrieval
failure, the same way `run_stream_task` already did.
...
| 8.2–8.3 live ingest eval (INGEST-01) | **Keep as-is *(this PR)*** | Depends on 8.0b; genuine anti-theater gate — live Ollama extract, no `extract_json` seed |
...
| Slice | Scope | Harness |
|-------|-------|---------|
| **10.1 *(implemented, narrower scope)*** | `checkpoints` table (new, no migration) captures a session's undo-journal watermark (`aether_permissions::current_undo_watermark`) plus its session-log turn count. `create_checkpoint`/`rewind_to_checkpoint` (`aether-daemon::checkpoint`), exposed as daemon IPC methods, undo every journal entry recorded since the watermark (`undo_since`, a generalization of UNDO-01's `undo_pending_writes`) *and* truncate the session log back to the checkpoint's turn count in the same call — restoring files and the transcript together, as required. **Narrower than planned:** this is an explicit, caller-invoked checkpoint/rewind pair, not an automatic checkpoint taken before every single mutating tool call — `undo_journal` already gives that per-write granularity; a checkpoint is the coarser, named, rewindable-later point on top of it. Symlink/hardlink limits are whatever `ProductionSandbox`/`undo_journal` already impose (documented in `docs/SANDBOX.md`), not a new mechanism. **CKPT-01** harness: checkpoint after turn 1, two more turns, rewind removes exactly the post-checkpoint files and truncates the log to 1 turn; a turn after rewind resumes numbering at 2 (proof truncation is real on disk); a second rewind to the same checkpoint is safe and also discards work added after the first rewind; rewinding an unknown checkpoint id fails closed | **+1 → 26 (implemented; Darwin canonical verified (run `30840008383` @ `51658d0`) — lands alongside HOOK-01 and CONS-01, see the scoreboard projection below for the combined total)** |
| **10.2** | Session resume / fork / ephemeral side-branch over the JSONL log | **FORK-01** *(probe)* |
| **10.3-10.4 *(implemented, narrower scope, out of order)*** | New `ToolInvocation::SubagentTask { paths }` step type, backed by `aether_core::subagent::run_subagent_read_task`: reads a bounded batch of files under its own file-count budget (`MAX_SUBAGENT_FILES = 20`, independent of the parent's `max_iterations`) and returns one distilled summary (path + byte count + bounded preview per file, capped at ~2k characters total as a conservative proxy for the roadmap's "≤2k tokens" target) as the single tool observation — not each file's full content. **Narrower than planned:** the distillation is mechanical (byte counts + previews), not an LLM-generated semantic summary; a real summarizer is a documented follow-up that can slot into the same contract (bounded output, separate budget) without changing it. **SUB-01** harness: delegating 8 files (24KB total) returns a summary under half that size while still naming every file, and the parent's own token accounting tracks only the summary's size, not the raw content — the actual "measured parent-context saving" this slice's anti-theater rule demands, not just a second loop call; a batch over the file budget fails as a normal failed observation (not a hard loop abort), closed not silent | **+1 → 29 (implemented; Darwin canonical verified (run `30840008383` @ `51658d0`))** |
...
| "Subagents" | SUB-01 measured parent-context saving, not a second loop call — **shipped** with mechanical (not LLM) distillation |
...
| **11.8 *(merged / prior PR)*** | **INJECT-01**: frozen corpus where tool *results* attempt to induce unplanned tool calls — cross-call correlation flags and blocks via `admit_plan_against_observations` (wired into LOOP-04 replan); tool output delimited at the session-log boundary (11.7 bundled) | **+1 → 32** |
...
| Phase 9 slices 9.7–9.8 *(merged)* | 23/23 | + UNDO-01 (session-scoped `undo_pending_writes`; git_init enumerated non-undoable); Darwin canonical verified (run `30840008383` @ `51658d0`) |
| Phase 9 slices 9.9–9.10 *(merged)* | 24/24 | + LOOP-04 (bounded replan on verify failure via `run_structured_with_replan`); Darwin canonical verified (run `30840008383` @ `51658d0`) |
| Phase 10 slice 10.7 *(merged, out of order)* | 25/25 | + HOOK-01 (one hard-coded `PreToolUse` path-denylist rule, not the full hook engine); Darwin canonical verified (run `30840008383` @ `51658d0`). Landed ahead of the rest of Phase 10 |
| Phase 10 slice 10.1 *(merged, out of order)* | 26/26 | + CKPT-01 (`create_checkpoint`/`rewind_to_checkpoint`, explicit not automatic-per-step); Darwin canonical verified (run `30840008383` @ `51658d0`). Landed alongside HOOK-01 |
...
| Phase 11 slices 11.7–11.8 *(prior PR)* | 32/32 | + INJECT-01 (untrusted tool-result delimiters + cross-call correlation on LOOP-04 replan; ≥8 deny + benign allow corpus); Linux-verified, Darwin canonical run pending |
| Phase 8.1–8.3 *(this PR)* | 33/33 | + INGEST-01 (live Ollama graph_extract on fresh transcript; schema-constrained decode; no extract_json seed); Linux-verified with Ollama, Darwin canonical pending |
...
| **Phase 11 complete** | **32/32** | CONS-01, SEC-01, SKILL-03, and INJECT-01 (non-probe Phase 11 scoreboard items) |
