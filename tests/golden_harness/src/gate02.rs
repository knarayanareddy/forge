...
struct Gate02Fixture {
    channel_id: String,
    session_id: String,
    channel_type: String,
    user_text: String,
    telegram_payload: serde_json::Value,
    loop_plan: Vec<serde_json::Value>,
}

pub fn gate02_fixture_ready() -> Result<(), String> {
    let raw = include_str!("../fixtures/gate02_telegram.json");
    let fixture: Gate02Fixture = serde_json::from_str(raw)
        .map_err(|e| format!("GATE-02 fixture parse failed: {}", e))?;
    if fixture.channel_id.is_empty() || fixture.loop_plan.is_empty() {
        return Err("GATE-02 fixture missing channel_id or loop_plan".into());
    }
    Ok(())
}

pub async fn test_gate02_impl(db: &Database) -> Result<(), String> {
    let raw = include_str!("../fixtures/gate02_telegram.json");
    let fixture: Gate02Fixture = serde_json::from_str(raw)
    ...
    let payload = serde_json::to_string(&fixture.telegram_payload).map_err(|e| e.to_string())?;

    {
        let conn = db.conn();
        conn.execute(
            "INSERT INTO sessions (id, title, status) VALUES (?1, 'GATE-02', 'active')",
        ...
        let denied = aether_daemon::gateway::mock_server::handle_mock_telegram_post(
            &conn,
            &fixture.channel_id,
            &payload,
        )?;
    ...
    let denied_status = post_mock_telegram_tcp(db, &fixture.channel_id, &payload).await?;
...

    {
        ...
        let accepted = aether_daemon::gateway::mock_server::handle_mock_telegram_post(
            &conn,
            &fixture.channel_id,
            &payload,
        )?;
    ...
    let parsed: serde_json::Value =
        serde_json::from_str(&body).map_err(|e| format!("GATE-02 response artifact is not JSON: {e}"))?;
    if parsed
        .get("reply")
        .and_then(|v| v.as_str())
        .unwrap_or("")
        ...
        .is_empty()
    {
        return Err("GATE-02 response artifact carries an empty reply".into());
    }
    if !parsed
        .get("artifacts")
    ...
    {
        return Err(format!(
            "GATE-02 response artifact does not name gate_marker.txt: {body}"
        ));
    }
    if body.contains(fixture.user_text.as_str()) {
        return Err(format!(
            "GATE-02 response artifact echoes the raw inbound user text — the pre-GATE-03 contract: {body}"
        ));
    }

    // Granted inbound via localhost mock server → run_task → response artifact.
    let ok_status = post_mock_telegram_tcp(db, &fixture.channel_id, &payload).await?;
    if ok_status != 200 {
        return Err(format!(
    ...
    if !response.exists() {
        return Err("GATE-02 response artifact missing after mock server round-trip".into());
    }
...
async fn post_mock_telegram_tcp(
    db: &Database,
    channel_id: &str,
    body: &str,
) -> Result<u16, String> {
    ...
    let path = format!("/gateway/telegram/{}", channel_id);
    let req = format!(
