...
use aether_core::{
    audit_loop_cost, ollama_token_usage, openai_token_usage, record_provider_token_usage,
    LoopConfig, LoopStreamEvent, ModelBackend, ModelRouter, OllamaProvider, ProviderTokenUsage, LoopCostAttribution,
};
...
fn test_frozen_turn_audit() -> Result<(), String> {
    ...
    audit_loop_cost(&conn, "sess-cost01", &aether_core::LoopCostAttribution { phase: "cost01_summary".into(), iteration: None, tool: None }, usage, "cost01")
        .map_err(|e| e.to_string())?;
    ...
    let count: i64 = conn
        ...
        .map_err(|e| e.to_string())?;
    if count < 1 {
        return Err(format!("COST-01 expected loop_token_usage audit, got {count}"));
    }

    let cost_count: i64 = conn
        .query_row(
            "SELECT COUNT(*) FROM audit_log WHERE session_id = 'sess-cost01' AND tool_name = 'loop_cost'",
            [],
            |row| row.get(0),
        )
        .map_err(|e| e.to_string())?;
    if cost_count < 1 {
        return Err("COST-01 expected loop_cost audit row".into());
    }
...
pub async fn test_cost01_impl() -> Result<bool, String> {
    ...
    if std::env::var("COST01_LIVE_OLLAMA").ok().as_deref() == Some("1") {
